...
import { saveCloseupProductMapping } from '@/app/admin/products/actions';
import type {
...
type CloseupProductMappingProps = {
...
export function CloseupProductMapping({
  unmappedRows,
  mappedRows,
  productOptions,
  marketGroupOptions,
}: CloseupProductMappingProps) {
  ...
  if (unmappedRows.length === 0 && mappedRows.length === 0) {
    return (
      <div className="rounded-[24px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <p className="text-sm text-slate-600">No Closeup products detected yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[24px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Closeup Mapping Process</p>
        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-900">
              Mapped Closeup Products ({mappedRows.length})
            </p>
          </div>
                            ...
                            onClick={() => {
                              startTransition(async () => {
                                try {
                                  ...
                                  await saveCloseupProductMapping(payload);
                                  ...
                                  setFeedback(
                                    error instanceof Error
                                      ? error.message
                                      : 'Unable to update Closeup mapping.',
                                  );
                                }
                              });
                            }}
                ...
                <tr>
                  <td colSpan={5} className="px-3 py-4 text-sm text-slate-600">
                    {mappedRows.length === 0
                      ...
                      : 'No mapped Closeup products match the selected market group.'}
                  </td>
                </tr>
              ) : null}
            </tbody>
      ...
      <div className="rounded-[24px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        ...
        >
          <div>
            <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Closeup Mapping</p>
            <p className="mt-1 text-sm font-medium text-slate-900">
              Unmapped Closeup Products ({unmappedRows.length})
            </p>
          </div>
          <span className="text-slate-600">
              ...
              <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-700">
                    Select multiple unmapped Closeup products and assign the same market group in one step.
                  </p>
                </div>
              ...
              <div className="mt-3 flex flex-col gap-3 xl:flex-row xl:items-center">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setFeedback(
                      `Assigned market group "${batchMarketGroup}" to ${selectedUnmappedCount} selected Closeup products in the form.`,
                    );
                  }}
                      ...
                      try {
                        ...
                        await Promise.all(
                          selectedRows.map(async (row) => {
                            const payload = new FormData();
                            payload.set('sourceProductName', row.sourceProductName);
                            payload.set('productId', selectedBySource[row.sourceProductName] ?? '');
                            payload.set(
                              'marketGroup',
                        ...
                        setFeedback(
                          `Batch mapping saved for ${selectedRows.length} Closeup products with market group "${batchMarketGroup}".`,
                        );
                        ...
                        setFeedback(
                          error instanceof Error
                            ? error.message
                            : 'Unable to save batch Closeup mapping.',
                        );
                      }
                    });
                  }}
                            ...
                            startTransition(async () => {
                              try {
                                ...
                                setFeedback(
                                  error instanceof Error
                                    ? error.message
                                    : 'Unable to save Closeup mapping.',
                                );
                              }
                            });
                          }}
                ...
                {unmappedRows.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-3 py-4 text-sm text-slate-600">
                      All detected Closeup products are already mapped.
                    </td>
                  </tr>
                ) : null}
              </tbody>
