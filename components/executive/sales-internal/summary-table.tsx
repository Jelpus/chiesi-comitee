import type { SalesInternalSummaryRow } from '@/types/sales-internal';

type SalesInternalSummaryTableProps = {
  rows: SalesInternalSummaryRow[];
};
...
export function SalesInternalSummaryTable({ rows }: SalesInternalSummaryTableProps) {
  if (rows.length === 0) {
    return (
      <div className="rounded-[24px] border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.05)]">
        <p className="text-sm text-slate-600">No summary data for the selected filters.</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.05)]">
      <div className="border-b border-slate-200 px-5 py-4">
        <h2 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-700">Summary</h2>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            {rows.map((row, index) => (
              <tr
                key={`${row.periodMonth}-${row.bu}-${row.channel}-${row.distributionChannel}-${index}`}
