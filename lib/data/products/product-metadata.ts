...
export type SanctionsProductMappingRow = CloseupProductMappingRow;
export type SanctionsUnmappedProductRow = CloseupUnmappedProductRow;
...
let ensureSanctionsProductMappingTablePromise: Promise<void> | null = null;
...
async function ensureSanctionsProductMappingTable() {
  if (!ensureSanctionsProductMappingTablePromise) {
    ensureSanctionsProductMappingTablePromise = (async () => {
      const client = getBigQueryClient();
      await client.query({
        query: `
  ...
  await ensureSanctionsProductMappingTablePromise;
}
...
export async function upsertSanctionsProductMapping(input: {
  sourceProductName: string;
  productId?: string;
  marketGroup?: string;
  isActive?: boolean;
  ...
  await ensureSanctionsProductMappingTable();
  const client = getBigQueryClient();
  const query = `
    MERGE \`chiesi-committee.chiesi_committee_admin.sanctions_product_mapping\` AS target
    USING (
      SELECT
        @sourceProductName AS source_product_name,
        @sourceProductNameNormalized AS source_product_name_normalized,
...
export async function getSanctionsProductMappings(limit = 1000): Promise<SanctionsProductMappingRow[]> {
  await ensureSanctionsProductMappingTable();
  const client = getBigQueryClient();
  const query = `
    WITH source_keys AS (
...
export async function getSanctionsUnmappedProducts(
  limit = 200,
): Promise<SanctionsUnmappedProductRow[]> {
  await ensureSanctionsProductMappingTable();
  const client = getBigQueryClient();
  const query = `
    WITH normalized_mapping AS (
      SELECT
        ...
        market_group,
        is_active
      FROM \`chiesi-committee.chiesi_committee_admin.sanctions_product_mapping\`
    ),
    source_columns AS (
      SELECT
        s.source_product_raw AS source_product_name,
...
export async function getSharedMarketGroups(limit = 500): Promise<string[]> {
  await Promise.all([
    ...
    ensureSanctionsProductMappingTable(),
  ...
  const query = `
    WITH all_groups AS (
      ...
      UNION ALL
      SELECT market_group FROM \`chiesi-committee.chiesi_committee_admin.sanctions_product_mapping\`
      UNION ALL
