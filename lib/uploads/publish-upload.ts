...
async function publishBusinessExcellenceCuotasUpload(context: UploadPublishContext) {
  const client = getBigQueryClient();
  const summaryModuleCode = 'business_excellence_cuotas';
  const cuotasEnrichedView =
    'chiesi-committee.chiesi_committee_stg.vw_business_excellence_cuotas_enriched';

  await client.query({
    query: `
      CREATE OR REPLACE VIEW \`${cuotasEnrichedView}\` AS
    ...
    query: `
      ...
      SELECT
        ...
        CONCAT(
          COALESCE(NULLIF(c.resolved_product_id, ''), CONCAT('market_', LOWER(REPLACE(COALESCE(c.market_group, 'unassigned'), ' ', '_')))),
          '_',
          LOWER(COALESCE(NULLIF(c.business_unit, ''), 'unknown')),
          '_',
          LOWER(REGEXP_REPLACE(COALESCE(NULLIF(c.channel, ''), 'unknown'), r'[^a-zA-Z0-9]+', '_'))
        ),
        CONCAT(
          CASE
            WHEN c.resolved_product_id = 'COMPETITOR' OR c.resolved_product_id IS NULL OR TRIM(c.resolved_product_id) = ''
          ...
          END,
          ' - ',
          COALESCE(NULLIF(c.business_unit, ''), 'Unknown'),
          ' - ',
          COALESCE(NULLIF(c.channel, ''), 'Unknown')
        ),
        SUM(CAST(c.quota_value AS NUMERIC)),
        CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC),
        CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC),
        'neutral',
        FALSE,
        CURRENT_TIMESTAMP(),
        NULL
      FROM \`${cuotasEnrichedView}\` c
      LEFT JOIN \`chiesi-committee.chiesi_committee_core.dim_module\` AS dm
        ON dm.module_code = @moduleCode
      WHERE c.reporting_version_id = @reportingVersionId
      GROUP BY dm.module_name, c.resolved_product_id, c.canonical_product_name, c.market_group, c.business_unit, c.channel
    `,
    params: {
      moduleCode: summaryModuleCode,
      periodMonth: context.periodMonth,
...
async function ensureCommercialOperationsOtifEnrichedView() {
  const client = getBigQueryClient();
  await client.query({
    query: `
...
async function publishCommercialOperationsIncidenciasUpload(context: UploadPublishContext) {
  const client = getBigQueryClient();
  const summaryModuleCode = 'commercial_operations_incidencias';

  await ensureCommercialOperationsOtifEnrichedView();
  ...
  await client.query({
    query: `
      ...
      SELECT
        ...
        @moduleCode,
        COALESCE(dm.module_name, @moduleCode),
        CONCAT('otif_', LOWER(REGEXP_REPLACE(COALESCE(NULLIF(channel_group, ''), 'other'), r'[^a-zA-Z0-9]+', '_'))),
        CONCAT('OTIF - ', COALESCE(NULLIF(channel_group, ''), 'Other')),
        CAST(SAFE_DIVIDE(COUNTIF(otif), NULLIF(COUNT(1), 0)) * 100 AS NUMERIC),
        CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC),
        CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC),
        CAST(SAFE_DIVIDE(COUNTIF(otif), NULLIF(COUNT(1), 0)) * 100 AS NUMERIC),
        'neutral',
      ...
      LEFT JOIN \`chiesi-committee.chiesi_committee_core.dim_module\` dm
        ON dm.module_code = @moduleCode
      WHERE s.upload_id = @uploadId
      GROUP BY dm.module_name, channel_group
    `,
    params: {
      uploadId: context.uploadId,
      moduleCode: summaryModuleCode,
  ...
  const [countRows] = await client.query({
    query: `
      SELECT COUNT(1) AS total
      FROM \`chiesi-committee.chiesi_committee_stg.stg_commercial_operations_incidencias\`
      WHERE upload_id = @uploadId
    `,
    params: { uploadId: context.uploadId },
  });
...
async function ensureCommercialOperationsSanctionsEnrichedView() {
  const client = getBigQueryClient();
  await client.query({
    query: `
      CREATE TABLE IF NOT EXISTS \`chiesi-committee.chiesi_committee_admin.sanctions_product_mapping\` (
...
async function publishCommercialOperationsSanctionsUpload(context: UploadPublishContext) {
  const client = getBigQueryClient();
  const summaryModuleCode = 'commercial_operations_sanctions';

  await ensureCommercialOperationsSanctionsEnrichedView();
  ...
  await client.query({
    query: `
      ...
      SELECT
        ...
        @moduleCode,
        COALESCE(dm.module_name, @moduleCode),
        CONCAT('sanctions_amount_', LOWER(REGEXP_REPLACE(COALESCE(NULLIF(channel_group, ''), 'other'), r'[^a-zA-Z0-9]+', '_'))),
        CONCAT('Sanctions Amount - ', COALESCE(NULLIF(channel_group, ''), 'Other')),
        CAST(COALESCE(SUM(sanction_amount), 0) AS NUMERIC),
        CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC),
        CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC), CAST(NULL AS NUMERIC),
        CAST(COUNT(1) AS NUMERIC),
        'neutral',
  ...
  const [countRows] = await client.query({
    query: `
      SELECT COUNT(1) AS total
      FROM \`chiesi-committee.chiesi_committee_stg.stg_commercial_operations_sanctions\`
      WHERE upload_id = @uploadId
    `,
    params: { uploadId: context.uploadId },
  });
...
export async function publishUploadToMart(uploadId: string) {
  ...
  if (context.moduleCode === 'business_excellence_cuotas' || context.moduleCode === 'cuotas') {
    return publishBusinessExcellenceCuotasUpload(context);
  }
  if (
  ...
  if (
    context.moduleCode === 'commercial_operations_incidencias' ||
    context.moduleCode === 'commercial_operations_otif' ||
    context.moduleCode === 'otif'
  ) {
    return publishCommercialOperationsIncidenciasUpload(context);
  }
  if (context.moduleCode === 'commercial_operations_sanctions' || context.moduleCode === 'sanctions') {
    return publishCommercialOperationsSanctionsUpload(context);
  }
  if (
